...
How to run locally:
1. Ensure Qdrant is reachable locally, e.g. through port-forward:
   kubectl port-forward svc/qdrant -n athenarc 6333:6333

2. Start this test app (not the main API):
   QDRANT_HOST=localhost QDRANT_PORT=6333 uvicorn internal_ap_flow:app --reload --app-dir tests
...
async def internal_recommend_logic(request: RecsRequest):
    """
    The core recommendation engine logic using Qdrant directly without authorization checks.
    """
    entity_id = request.entity_id
    n = request.n

    try:
        recs_list = qdrant_client.get_recommendations(
            application="ds2ds",
            entity_id=entity_id,
            limit=n,
        )
...
async def test_get_recommendations_ap(analytical_pattern: Dict):
    ...
    try:
        ...
        # 3. Inject results back into the Graph JSON
